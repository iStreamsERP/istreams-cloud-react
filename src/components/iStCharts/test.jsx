...
const DashboardPage = () => {
  ...
  const fetchUserData = useCallback(async () => {
    if (!dashboardId || !userData?.clientURL) {
      console.warn("Missing dashboardId or clientURL");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (master && master.length > 0) {
        ...
        if (!savedLayout && layoutFromAPI && layoutFromAPI > 0) {
          ...
          localStorage.setItem("selected_layout", layoutFromAPI.toString());
        }
      }

      // Set chatbot context safely
      if (event && event.length > 0) {
        localStorage.setItem("chatbot_context", JSON.stringify({
          source: "events",
          title: "Upcoming Events", // Fixed: item was not defined here
          data: event,
        }));
      }
    } catch (error) {
  ...
  }, [dashboardId, userData?.clientURL]);
  ...
  // Effect to handle layout changes and ensure data is loaded
  useEffect(() => {
    if (!dataLoaded && !loading && dashboardId) {
      fetchUserData();
    }
  }, [selectedLayout, dataLoaded, loading, dashboardId, fetchUserData]);

  // Handle layout change
  const handleLayoutChange = (newLayout) => {
    setSelectedLayout(newLayout);
    localStorage.setItem("selected_layout", newLayout.toString());
    
    // Force a small delay to ensure state is updated before any re-rendering
    setTimeout(() => {
      if (!dataLoaded || dbData.length === 0) {
        fetchUserData();
      }
    }, 100);
  };
